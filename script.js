...
async function processSale() {
    ...
    const finalPrice = parseFloat(document.getElementById('finalSalePrice').value);
    const modalMsg = document.getElementById('modalMessage');
    if(isNaN(finalPrice)||finalPrice<=0){ modalMsg.textContent="❌ Prix invalide"; return; }

    const snapshot = await db.collection(STOCK_COLLECTION).where('id_ordinateur','==',currentPcId).get();
    if(snapshot.empty) return;
    const pcDoc = snapshot.docs[0];
    await db.collection(STOCK_COLLECTION).doc(pcDoc.id).update({
        statut: 'Vendu',
        prix_vente_final: finalPrice,
        date_vente: new Date().toLocaleDateString('fr-FR')
    });
    modalMsg.textContent = `✅ Vente enregistrée !`;
    renderStock();
    updateDashboard();
...
async function confirmDeletePc(){
    ...
    renderStock();
    updateDashboard();
    document.getElementById('message').textContent = `✅ Article N° ${formatInventoryId(currentPcId)} supprimé.`;
}
